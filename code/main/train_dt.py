#!/usr/bin/env python3
"""
train_dt.py - Decision Transformer训练脚本（直接使用 CSV 数据）
- 输入目录应包含 train.csv 和 val.csv（由 split_data.py 生成）
- 添加详细计时日志以诊断启动卡顿问题
"""
...
def parse_arguments():
    ...
    parser.add_argument('--data_dir', type=str, required=True,
                        help='预处理数据目录，应包含 train.csv 和 val.csv')
    ...
    try:
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                curr_score_input = curr_score[:, :-1, :]
                curr_score_target = curr_score[:, 1:, :]
...
def train_dt_with_eval(device, step_num, state_dim, train_csv, val_csv,
    ...
    # ========== 计时：加载训练数据 ==========
    t0_load_start = time.time()
    logger.info("开始加载训练数据...")

    # 注意：如果需要更细粒度的计时，请在 utils.py 的 EpisodeReplayBuffer.__init__ 中添加计时。
    # 例如：
    # def __init__(self, ...):
    #     t0 = time.time()
    #     df = pd.read_csv(data_path)
    #     t1 = time.time(); print(f"读取CSV耗时: {t1-t0:.2f}s")
    #     # 解析 state...
    #     t2 = time.time(); print(f"解析state耗时: {t2-t1:.2f}s")
    #     # 构建轨迹...
    #     t3 = time.time(); print(f"构建轨迹耗时: {t3-t2:.2f}s")
    #     # 计算采样权重...
    #     t4 = time.time(); print(f"计算权重耗时: {t4-t3:.2f}s")
    ...
    t0_load_end = time.time()
    logger.info(f"训练数据加载完成，总耗时: {t0_load_end - t0_load_start:.2f} 秒")
    ...
    try:
        ...
        norm_dict = {"state_mean": state_mean, "state_std": state_std}
    ...
    t_model_init_end = time.time()
    logger.info(f"模型初始化耗时: {t_model_init_end - t_model_init_start:.2f} 秒")

    # 计时：构建采样器和 DataLoader
    t_sampler_start = time.time()
    sampler = WeightedRandomSampler(
        replay_buffer.p_sample,
        num_samples=step_num * batch_size,
        replacement=True
    )
    dataloader = DataLoader(replay_buffer, sampler=sampler, batch_size=batch_size)
    t_sampler_end = time.time()
    logger.info(f"采样器和 DataLoader 构建耗时: {t_sampler_end - t_sampler_start:.2f} 秒")
    ...
    for step_idx, (states, actions, rewards, dones, all_reward,
        ...
        # 验证评估
        if val_csv and step_idx % eval_step == 0:
            val_loss = evaluate_model(
                model, val_csv, device, state_dim, n_ctx,
                replay_buffer.state_mean, replay_buffer.state_std
            )
    ...
    if step_times:
        ...
        logger.info(f"平均每步耗时: {avg_step_time:.4f} 秒")
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("Decision Transformer 训练（直接使用 CSV 数据）")
    logger.info("=" * 80)
    ...
    if not os.path.exists(train_csv):
        logger.error(f"训练文件不存在: {train_csv}")
        sys.exit(1)
    if not os.path.exists(val_csv):
        logger.error(f"验证文件不存在: {val_csv}")
        sys.exit(1)

    # 获取状态维度（可从 stats.json 读取，或保持参数）
    state_dim = args.state_dim
    stats_path = os.path.join(args.data_dir, 'stats.json')
    if os.path.exists(stats_path):
        with open(stats_path, 'r') as f:
            stats = json.load(f)
        state_dim = stats.get('state_dim', state_dim)
        logger.info(f"状态维度: {state_dim} (从 stats.json 加载)")
    ...
    model_param = {
        ...
        "state_dim": state_dim,
        "dir": train_csv,           # 传给 train_dt_with_eval 作为 train_csv
        "val_dir": val_csv,          # 传给 train_dt_with_eval 作为 val_csv
    ...
    try:
        train_dt_with_eval(
            ...
            train_csv=model_param["dir"],
            val_csv=model_param["val_dir"],
