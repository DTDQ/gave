...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        causal_mask_size = config['n_ctx'] * 3
        
        self.register_buffer("bias",
                             torch.tril(torch.ones(causal_mask_size, causal_mask_size))
                             .view(1, 1, causal_mask_size, causal_mask_size))
...
class GAVE(nn.Module):
    ...
    def __init__(self, state_dim, act_dim, state_mean, state_std, hidden_size=64, action_tanh=False, K=20,
                max_ep_len=144, scale=2000, warmup_steps=10000, weight_decay=0.0001, learning_rate=0.0001, time_dim=8,
        ...
        # ========== 初始化所有层 ==========
        # Transformer 模块
        self.transformer = nn.ModuleList([Block(block_config) for _ in range(block_config['n_layer'])]).to(self.device)
        
        # 嵌入层
        self.embed_timestep = nn.Embedding(self.max_ep_len, self.time_dim).to(self.device)
        self.embed_return = torch.nn.Linear(1, self.hidden_size).to(self.device)
        self.embed_reward = torch.nn.Linear(1, self.hidden_size).to(self.device)
        self.embed_state = torch.nn.Linear(self.state_dim, self.hidden_size).to(self.device)
        self.embed_action = torch.nn.Linear(self.act_dim, self.hidden_size).to(self.device)  # ✅ 添加
        
        # 转换层
        self.trans_return = torch.nn.Linear(self.time_dim + self.hidden_size, self.hidden_size).to(self.device)
        self.trans_reward = torch.nn.Linear(self.time_dim + self.hidden_size, self.hidden_size).to(self.device)
        self.trans_state = torch.nn.Linear(self.time_dim + self.hidden_size, self.hidden_size).to(self.device)
        self.trans_action = torch.nn.Linear(self.time_dim + self.hidden_size, self.hidden_size).to(self.device)
        
        # 层归一化
        self.embed_ln = nn.LayerNorm(self.hidden_size).to(self.device)
        
        # 预测层
        self.predict_state = torch.nn.Linear(self.hidden_size, self.state_dim).to(self.device)
        ...
        self.predict_beta = nn.Sequential(
            nn.Linear(self.hidden_size, 16).to(self.device),
            nn.GELU().to(self.device),
            nn.Linear(16, 8).to(self.device),
            nn.GELU().to(self.device),
            nn.Linear(8, 1).to(self.device),
            nn.Sigmoid().to(self.device),
        ).to(self.device)  # ✅ 整个序列加上
        
        self.predict_return = nn.Sequential(
            nn.Linear(self.hidden_size, 128).to(self.device),
            nn.GELU().to(self.device),
            nn.Linear(128, 16).to(self.device),
            nn.GELU().to(self.device),
    ...
    def forward(self, states, actions, rewards, returns_to_go, timesteps, attention_mask=None):
        ...
        with grad_context:
            ...
            # 注意：actions_1 是基于 detach 的 actions 计算的，因此不会有梯度传到 actions
            actions_1 = actions.clone().detach() * beta_preds
            
            # 重新计算 action_embeddings_1
            ...
            # 重新堆叠输入（使用修改后的 action_embeddings_1）
    ...
    def step(self, states, actions, rewards, dones, all_reward, curr_score, timesteps, attention_mask, next_states):
        ...
        return (loss.detach().cpu().item(), loss1.detach().cpu().item(), loss2.detach().cpu().item(), loss3.detach().cpu().item(),
                torch.mean(wo_frozen.squeeze()).cpu().item(), torch.mean(curr_score_target).cpu().item(),
    ...
    def take_actions(self, state, target_return=None, pre_reward=None, budget=100, cpa=2):
        ...
        if self.eval_states is None:
            if isinstance(state, np.ndarray):
                ...
                self.eval_states = state.reshape(1, self.state_dim).to(self.device)
            else:
            ...
            if isinstance(state, np.ndarray):
                ...
                cur_state = state.reshape(1, self.state_dim).to(self.device)
            else:
                raise TypeError(f"Unsupported state type: {type(state)}")
